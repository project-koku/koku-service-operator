...
main() {
    ...
    extract_client_secret
    assign_sync_client_realm_roles
    if [ "$SKIP_REALM_USERS" = "1" ]; then
        echo_info "Skipping realm user and org group provisioning (SKIP_REALM_USERS=1)"
    else
        create_realm_users
        create_org_groups
    fi
...
case "${COMMAND}" in
    ...
    "help"|"-h"|"--help")
        ...
        echo "  SKIP_REALM_USERS          Set to 1 to skip jwtAuth.realmUsers provisioning"
        echo "                            (realm, clients, and secrets are still created)"
        echo ""
