...
_apply_listener_cpu_deploy() {
    ...
    # missing path, so fall back to a strategic merge patch which uses the container
    # name as the merge key and therefore only updates the named container without
    # replacing or removing any other containers (e.g. sidecars).
    log_verbose "JSON patch failed (no existing resources block?); retrying with strategic merge patch"
    perf_kubectl patch deploy "${listener_deploy}" -n "${NAMESPACE}" --type=strategic -p "$(cat <<EOF
{
  "spec": {
    "template": {
      "spec": {
