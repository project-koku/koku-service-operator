...
Types are largely implemented and richer than the JIRA description, but COST-7678 is **not done**. Open ticket gaps: admission webhooks (G1), remaining CEL/OpenAPI validation (G2), and profile sizing maps (G4). G3 (`dataRetentionMonths`) is closed in [#16](https://github.com/project-koku/koku-service-operator/pull/16) — field on Spec with CRD default/Min/Max; reconciler wiring to `RETAIN_NUM_MONTHS` is in scope of that PR. Related API security gaps (`RealmUser.Password`, raw `Env` maps) also remain in the type surface.

**In-flight PRs (2026-08-13):**

| PR | Gap | Closes |
|----|-----|--------|
| [#16](https://github.com/project-koku/koku-service-operator/pull/16) | G3 | `spec.costManagement.dataRetentionMonths` (default 4, range 1–60); `RETAIN_NUM_MONTHS` wiring |
...
| `defaults.go` mutating webhook | **In progress** — [#52](https://github.com/project-koku/koku-service-operator/pull/52) (scaffold; `dataRetentionMonths` CRD default in [#16](https://github.com/project-koku/koku-service-operator/pull/16)) |
...
[#52](https://github.com/project-koku/koku-service-operator/pull/52) adds `costmanagementserviceconfig_webhook.go`, `SetupWebhookWithManager` in `cmd/main.go`, and `config/webhook/` + cert-manager kustomize overlays. Mutating webhook is a scaffold; `dataRetentionMonths` CRD default is added in [#16](https://github.com/project-koku/koku-service-operator/pull/16).
...
| `dataRetentionMonths` 1–60 | Done (OpenAPI Minimum/Maximum on `spec.costManagement.dataRetentionMonths`; [#16](https://github.com/project-koku/koku-service-operator/pull/16)) |
...
Field added on `CostManagementConfig` as `spec.costManagement.dataRetentionMonths` with CRD default `4` (aligned with Koku `RETAIN_NUM_MONTHS`; JIRA said 3) and range 1–60. Reconciler/env wiring to `RETAIN_NUM_MONTHS` is in [#16](https://github.com/project-koku/koku-service-operator/pull/16).
