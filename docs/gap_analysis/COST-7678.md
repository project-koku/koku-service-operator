# COST-7678 Gap Analysis: Define CostManagement CRD types

**Jira:** [COST-7678](https://redhat.atlassian.net/browse/COST-7678)
**Audited:** 2026-08-10
**Purpose:** Handoff audit of CRD type surface vs ticket acceptance criteria. This document does not update `docs/tasks.md` or other trackers.
**Beta scope:** ROS and Kruize are **out of beta** ([BETA-PRIORITY.md](BETA-PRIORITY.md)). Keeping `ros` / `kruize` Spec types and defaulted partition flags is fine; optional enablement and ROS/Kruize profile sizing → [COST-8054](../jira/COST-8054.md).

## Verdict

Types are largely implemented and richer than the JIRA description, but COST-7678 is **not done**. Remaining ticket gaps are admission webhooks, CEL/OpenAPI validation, profile sizing maps, and the missing `dataRetentionMonths` field. Related API security gaps (`RealmUser.Password`, raw `Env` maps) also remain in the type surface.

## Sources

- [Jira COST-7678](https://redhat.atlassian.net/browse/COST-7678)
- [docs/jira/COST-7678.md](../jira/COST-7678.md)
- [api/v1alpha1/costmanagementserviceconfig_types.go](../../api/v1alpha1/costmanagementserviceconfig_types.go)
- [docs/design/design-vs-jira.md](../design/design-vs-jira.md)

Out of scope per ticket: reconciler logic, resource builders.

---

## Ticket acceptance criteria

| Deliverable | Status |
|-------------|--------|
| Top-level Spec/Status types | Done (different kind/file name) |
| Infra types (DB, cache, Kafka, auth, object storage, monitoring) | Done (flat spec, not `spec.infra`) |
| App types + `dataRetentionMonths` | Partial — app surface exists; **`dataRetentionMonths` absent** |
| Status: phase, conditions, DiscoveredConfig | Done with intentional phase rename |
| `ComponentStatus` | Removed (intentional; conditions instead) |
| `profiles.go`: enum + resource sizing maps | Partial — enum only; **no sizing maps** |
| `defaults.go` mutating webhook | **Missing** |
| `validation.go` webhook + CEL | **Missing** |
| External-only infra (no type field) | Deviated — `deploy *bool` for DB/cache |

---

## Done (meets or supersedes ticket intent)

Implemented in [`api/v1alpha1/costmanagementserviceconfig_types.go`](../../api/v1alpha1/costmanagementserviceconfig_types.go):

- Full Spec surface: `profile`, `global`, `database`, `cache`, `kafka`, `objectStorage`, `auth`, `rbac`, `costManagement`, `ros`, `kruize`, `ingress`, `ui`, `gatewayRoute`, `monitoring`
- Status: `phase`, `conditions []metav1.Condition`, `discoveredConfig`, `observedGeneration`
- Condition constants: `Available`, `Progressing`, `Degraded`, plus `DiscoveryComplete`, `DatabaseReady`, `CacheReady`, `StorageReady`, `KafkaReady`, `AuthenticationReady`, `SchemaUpToDate`, and extra `UIReady` (not in JIRA)
- Profile enum `standard` / `ha` with CRD default `standard`
- Exactly **12** `*bool` defaulted fields with `BoolVal` helper:
  1. `ServiceAccountSpec.Create`
  2. `DatabaseConfig.Deploy`
  3. `CacheConfig.Deploy`
  4. `ObjectStorageConfig.UseSSL`
  5. `KruizePartitionsSpec.CreateEnabled`
  6. `KruizePartitionsSpec.DeleteEnabled`
  7. `ROSPartitionCleanerSpec.Enabled`
  8. `CostManagementConfig.ScheduleReportChecks`
  9. `KokuAPISpec.Enabled`
  10. `MasuSpec.Enabled`
  11. `ListenerSpec.Enabled`
  12. `MonitoringConfig.Enabled`
- `ComponentStatuses` removed — status uses `[]metav1.Condition` only
- Phase enum: `Pending` / `Progressing` / `Ready` / `Degraded`
- Deep-copy + CRD generated under `config/crd/bases/service.costmanagement.openshift.io_costmanagementserviceconfigs.yaml`
- Django secret-key charset fixed in [`internal/resources/secrets.go`](../../internal/resources/secrets.go) (not a CRD field; adjacent to ticket notes)

Partial defaulting already exists via CRD OpenAPI defaults (`+kubebuilder:default`) for `spec.profile` → `standard` and `spec.monitoring.enabled` → `true`. `dataRetentionMonths` cannot be defaulted because the field does not exist.

---

## Intentional deviations (do not treat as bugs)

Documented in [design-vs-jira.md](../design/design-vs-jira.md) §1–3:

| JIRA | Implementation | Rationale |
|------|----------------|-----------|
| Kind `CostManagement` | `CostManagementServiceConfig` (`cmsc`) | Naming settled for OLM/product |
| 6-file split | Single `costmanagementserviceconfig_types.go` | Semantically identical; file not large enough |
| Linear phases (`Discovering`…`Deploying`) | `Pending`/`Progressing`/`Ready`/`Degraded` + conditions | Conditions are the machine API |
| `spec.infra.*` / `spec.app.*` | Flat top-level fields | Current API shape |
| `credentialsSecretRef` | `secretName` / Kafka `sasl.existingSecret` | Simpler string refs |
| `authentication.issuerUrl` | `auth.keycloak.issuerURL` (+ `url`) | Keycloak-nested auth model |
| External-only infra | `database.deploy` / `cache.deploy` default true | Dev/CI only; production is BYOI |
| `ComponentStatus` | `metav1.Condition` | Kubernetes convention |

---

## Remaining gaps (ticket-scoped)

### G1. Admission webhooks — missing

No `api/v1alpha1/defaults.go`, no `api/v1alpha1/validation.go`, no `config/webhook/`, no `SetupWebhookWithManager` in `cmd/main.go`. Only commented `[WEBHOOK]` kustomize scaffolding and a `controller-gen ... webhook` Makefile flag that generates nothing without webhook types.

`internal/controller/validation.go` is **reconciler** connectivity/secret probing (COST-7684), not admission.

| Default | CRD OpenAPI today | Webhook still needed? |
|---------|-------------------|------------------------|
| `spec.profile` → `standard` | Yes | No for this value |
| `monitoring.enabled` → `true` | Yes (`spec.monitoring`, not `spec.infra.monitoring`) | No for this value |
| `dataRetentionMonths` → `3` | Field missing | Yes — after field added |

### G2. CEL / OpenAPI validation — missing

No `+kubebuilder:validation:XValidation`, no Min/Max on ports, no `sslMode` enum marker.

| JIRA rule | Status |
|-----------|--------|
| Required: `database.host`, `database.credentialsSecretRef`, `cache.host`, `cache.credentialsSecretRef` | Not CRD-required (and cannot be unconditionally required while `deploy: true` exists); API uses `secretName` |
| Required: `kafka.bootstrapServers` | CRD-required today |
| Required: `authentication.issuerUrl` | Not required; field is `auth.keycloak.issuerURL` optional |
| `database.port` 1–65535 | No Min/Max |
| `sslMode` ∈ disable/require/verify-ca/verify-full | Default `disable` only; no Enum |
| issuerUrl must be HTTPS | Not enforced |
| audiences non-empty | Not enforced |
| profile ∈ standard/ha | Done (Enum) |
| `dataRetentionMonths` 1–60 | Field missing |
| requests ≤ limits | Not enforced |

**Implication for BYOI:** validation must be conditional (`deploy == false` ⇒ host + secret required). That is webhook and/or CEL `XValidation`, not simple `required:`.

### G3. `dataRetentionMonths` — missing field

JIRA `app_types.go` and defaults/validation both require it. No references in Go types or CRD YAML. Either add to Spec (likely under `costManagement`) or explicitly defer/reject with a design note — today it is an untracked ticket gap.

### G4. Profile resource sizing maps — missing

`Profile` enum exists; **no** sizing maps in `api/` or `internal/` keyed by profile. `docs/tasks.md` attributes profile-based sizing to COST-7686/7693, but COST-7678 explicitly asks for maps in `profiles.go`. Treat as an API-package deliverable still open for 7678, even if reconciler consumption is later.

---

## Related gaps (not in Jira body; same surface)

### R1. API security gaps in types (design §4a/§4b; block beta/GA)

Not listed in the JIRA body, but they are part of the CRD type surface audited with this ticket:

- `RealmUser.Password string` still in Spec (`api/v1alpha1/costmanagementserviceconfig_types.go`) — **P0** for beta ([BETA-PRIORITY.md](BETA-PRIORITY.md) T2)
- `Env map[string]string` on Koku API / Masu / Listener — **P1** (tighten before GA; warn in beta docs)

---

## Naming / docs drift (note only — not fixed here)

These confuse audits but are left untouched by this gap analysis:

- `CLAUDE.md` still says `ComponentStatuses` is a known gap and documents `credentialsSecretRef`; types use `secretName` and conditions only
- Sample BYOI header comments still say `credentialsSecretRef`
- Local `docs/jira/COST-7678.md` Status says "To Do"; live Jira is In Progress
- `docs/tasks.md` marks only webhooks missing — incomplete vs G2–G4

---

## Recommended closure order

For the team member closing COST-7678:

1. **Add CRD markers that need no webhook runtime:** `sslMode` Enum, port Min/Max, optional CEL for requests≤limits and BYOI required-when-not-deployed.
2. **Decide `dataRetentionMonths`:** add field + default `3` + range 1–60, or formally defer out of 7678.
3. **Add profile sizing maps** in the API package (or `internal/resources`) and wire later in COST-7686/7693 (Celery/core first; ROS and Kruize maps → [COST-8054](../jira/COST-8054.md)).
4. **Scaffold admission webhooks** (`defaults.go` / `validation.go`, certs, `config/webhook`, main registration) for rules CEL cannot express (HTTPS issuer, audiences, sensitive `Env` keys, deploy+host contradictions).
5. **R1 (related / beta P0):** remove/replace `RealmUser.Password` and tighten `Env` before shipping the beta API.
