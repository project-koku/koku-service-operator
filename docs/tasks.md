...
| [COST-7696](https://redhat.atlassian.net/browse/COST-7696) | Build CI pipeline for bundle | ❌ | GitHub Actions CI with lint/build/test/check-generated/container-build ✅. Missing on GHA: bundle validation, scorecard tests. CatalogSource + OLM install live on Prow (`e2e-olm` / `e2e-pytest`; [openshift-ci.md](openshift-ci/openshift-ci.md); COST-7699). |
...
| [COST-7698](https://redhat.atlassian.net/browse/COST-7698) | Implement operator-specific E2E scenarios | 🔄 | `test/e2e/cmsc_*.go` + [cmsc-e2e.md](development/cmsc-e2e.md); OP-E2E-001–008 implemented; 009 stubbed (COST-7694). CI gate: COST-7699. |
| [COST-7699](https://redhat.atlassian.net/browse/COST-7699) | Set up OpenShift CI integration | 🔄 | Prow jobs in `openshift/release` (`build`/`images` auto-run; bundle manual `/test`; `e2e-olm`/`e2e-pytest`/`e2e-iqe` optional `/test`, not every PR). Docs: [openshift-ci.md](openshift-ci/openshift-ci.md). Remaining: required e2e gate, periodics, Go CMSC suite ([cmsc-e2e.md](development/cmsc-e2e.md)). |
