...
| [COST-7699](https://redhat.atlassian.net/browse/COST-7699) | Set up OpenShift CI integration | 🔄 | Prow jobs in `openshift/release` (`build`/`images`/`bundle` auto-run; `e2e-olm`/`e2e-pytest`/`e2e-iqe` optional `/test`, not every PR). Docs: [openshift-ci.md](openshift-ci/openshift-ci.md). Remaining: required e2e gate, periodics, Go CMSC suite ([cmsc-e2e.md](development/cmsc-e2e.md)). |
