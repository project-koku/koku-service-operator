...
| Create pod `iqe-cost-tests` | `ENV_FOR_DYNACONF=cost_onprem`, `--force-default-user cost_onprem_user`, marker `cost_required`, no `-k` filter | Image pull / Ready wait (5m) |

`--profile smoke` ([`scripts/lib/iqe-filters.sh`](../../scripts/lib/iqe-filters.sh)):

- Marker: `-m cost_required` (no `-k` filter)
...
| IQE smoke | 0 failed; `ENV_FOR_DYNACONF=cost_onprem` | Selected from the IQE image catalog via `-m cost_required` plus skip groups |
...
| `junit_iqe.xml` | `e2e-iqe` / iqe | IQE smoke (`-m cost_required`; `ENV_FOR_DYNACONF=cost_onprem`) |
